...
perfect = data.frame(year <- data$year, cfam <- data$cfam, age <- data$age, sex <- data$sex, a_stress <- data$BP1,  mh_stress <- data$mh_stress, sc1 <- data$BP6_10, stringsAsFactors = FALSE)
...
# 미성년자 제외 만 19세 이상
perfect <- perfect[perfect$age....data.age >= 19,]

#총 인원(조사에 임한인원 수 가족 구성원 수를 모른다고 말한 인원 제외 19852명)
total = length(which(perfect$cfam....data.cfam != 9)) #18305
# 1인가구 수 (2282)
...
sc1_1 = length(which(OPH$sc1....data.BP6_10== 1))
bp1_1 = length(which(OPH$a_stress....data.BP1 == 1))
89/119
89/2282

MPH <- perfect[perfect$cfam....data.cfam > 1 & perfect$cfam....data.cfam != 9,]
sc1_2 = length(which(MPH$sc1....data.BP6_10 == 1))
bp1_2 = length(which(MPH$a_stress....data.BP1 == 1))
219/747
219/16023
